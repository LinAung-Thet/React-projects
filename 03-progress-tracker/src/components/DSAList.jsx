...
export default function DSAList() {
    ...
    useEffect(() => {
        const localList = JSON.parse(localStorage.getItem("dsalist1")) || [];
        setDsaList(localList.length !== 0 ? localList : dsaTrackerList);
        console.log("Loaded from local store: " + localList.length !== 0);
    }, []);
    ...
    return (
        <div className="flex flex-col gap-10 w-[60%] mb-40 relative">
            ...
            {dsaList.map((section1, index) => {
                console.log(section1.subsections);
                return (
                    <Section
                        index={index}
